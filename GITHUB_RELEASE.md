...
- Manual install: download `RehearsalPlayer-1.1.13.zip` and extract the `RehearsalPlayer` folder into your LMS plugins directory
...
1. Commit `repo.xml`, `release-assets/RehearsalPlayer-1.1.13.zip`, and `release-assets/RehearsalPlayer-1.1.13.sha1` to `jdmediatv/LyrionRehearsalPlayer` main.
2. Create release tag `v1.1.13`.
3. Upload `RehearsalPlayer-1.1.13.zip` and `RehearsalPlayer-1.1.13.sha1` as release assets (optional - the raw `release-assets/` copy in the repo is what `repo.xml` actually points to and is sufficient for installs to update).
